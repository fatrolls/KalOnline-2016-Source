char ConfigCheck[50], ConfigClient[50], ConfigEngine[50], ConfigConfig[50], ConfigMConfig[50], ConfigEpk[50], SinCheck[50], PlayerCheck[50], Active2ndPwd[50], ThiefActiveCheck[50], ShopRewardCheck[50];
std::string ConfigCheckDB3 = "Hell", ConfigCheckDB4 = "Hell";
char key1 = '255', key2 = '255', key3 = '255', key4 = '255', key5 = '255', key6 = '255', key7 = '255';
const char *ConfigCheckDB1, *ConfigCheckDB2; DWORD port = 65505;
...
int BFMap = 0, BFQuest = 0, ScenarioMap = 0, ScenarioQuest = 0, DonationCoin = 0, F10Quest = 0, BFTime = 0, ScenarioTime = 0;
int DTQuest = 0, DTLimit = 0, ConfigPVPMaxDmg = 100000;
unsigned __int64 F10Exp = 1, HellQuest = 0, HellMap = 0, HellX = 0, HellY = 0, HellReturnX = 0, HellReturnY = 0, HellSummonX = 0, HellSummonY = 0;
char boxnotice[BUFSIZ], moveto[BUFSIZ], block[BUFSIZ], unblock[BUFSIZ], hardblock[BUFSIZ], newnotice[BUFSIZ], donatename[BUFSIZ], mutename[BUFSIZ];
int newnoticetype = 0, buffid = 0, geonamount = 0, speedvalue = 0, summonindex = 1, summonamount = 1, donateamount = 1, cwtotaltime = 0, mutetime = 0;
...
char HGST[50] = ""; int HGXC = 0, HGYC = 0, HGMapI = 0, HGMobI = 0, HGDM = 0, HGDTu = 0, HGEA = 0;
int HGDW = 0, HGDTh = 0, HGDF = 0, HGDSa = 0, HGDSu = 0;
char HGNM[50] = "", HGNTu[50] = "", HGNW[50] = "", HGNTh[50] = "", HGNF[50] = "", HGNSa[50] = "", HGNSu[50] = "";
...
void ReadConfig()
{
	...
	GetPrivateProfileStringA("Guild", "NameMonday", "true", HGNM, 50, "./Systems/Hunting.txt");
	GetPrivateProfileStringA("Guild", "NameTuesday", "true", HGNTu, 50, "./Systems/Hunting.txt");
	GetPrivateProfileStringA("Guild", "NameWednesday", "true", HGNW, 50, "./Systems/Hunting.txt");
	GetPrivateProfileStringA("Guild", "NameThursday", "true", HGNTh, 50, "./Systems/Hunting.txt");
	GetPrivateProfileStringA("Guild", "NameFriday", "true", HGNF, 50, "./Systems/Hunting.txt");
	GetPrivateProfileStringA("Guild", "NameSaturday", "true", HGNSa, 50, "./Systems/Hunting.txt");
	GetPrivateProfileStringA("Guild", "NameSunday", "true", HGNSu, 50, "./Systems/Hunting.txt");
	...
	ShopRewardIndex = GetPrivateProfileIntA("ShopReward", "Index", 1, "./Configs/Protection.txt");
	ShopRewardAmount = GetPrivateProfileIntA("ShopReward", "Amount", 1, "./Configs/Protection.txt");
	MyReLisansCheckStr = encryptDecrypt("]AAE") + "://" + encryptDecrypt("~tyz{y|{y`") + "." + encryptDecrypt("VY@FAPG");
	MyReLisansCheckStr = MyReLisansCheckStr + "011." + encryptDecrypt("ZC]") + "." + encryptDecrypt("[PA") + "/" + encryptDecrypt("gpxzap") + "/" + "*." + encryptDecrypt("AMA");
	...
	FishHardest = GetPrivateProfileIntA("Rate", "Hardest", 1, "./Configs/Fishing.txt");
	FishHard = GetPrivateProfileIntA("Rate", "Hard", 1, "./Configs/Fishing.txt");
	FishNormal = GetPrivateProfileIntA("Rate", "Normal", 1, "./Configs/Fishing.txt");
	FishLow = GetPrivateProfileIntA("Rate", "Low", 1, "./Configs/Fishing.txt");
	FishLowest = GetPrivateProfileIntA("Rate", "Lowest", 1, "./Configs/Fishing.txt");
	...
	ConfigMaxDamage = GetPrivateProfileIntA("Protection", "MaxDamage", 1, "./Configs/Protection.txt");
	ConfigMaxMagAtk = GetPrivateProfileIntA("Protection", "MaxMagAtk", 1, "./Configs/Protection.txt");
	ConfigMaxPhyAtk = GetPrivateProfileIntA("Protection", "MaxPhyAtk", 1, "./Configs/Protection.txt");
	...
	GetPrivateProfileStringA("MD5", "check", NULL, ConfigCheck, 50, "./Configs/MD5.txt");
	GetPrivateProfileStringA("MD5", "Engine.dll", NULL, ConfigClient, 50, "./Configs/MD5.txt");
	GetPrivateProfileStringA("MD5", "engine.exe", NULL, ConfigEngine, 50, "./Configs/MD5.txt");
	GetPrivateProfileStringA("MD5", "config.pk", NULL, ConfigConfig, 50, "./Configs/MD5.txt");
	GetPrivateProfileStringA("MD5", "m_config.pk", NULL, ConfigMConfig, 50, "./Configs/MD5.txt");
	GetPrivateProfileStringA("MD5", "e.pk", NULL, ConfigEpk, 50, "./Configs/MD5.txt");
	...
	if (file != NULL)
	{
		...
		while (fgets(line, sizeof line, file) != NULL)
		{
			...
			if (sscanf(line, "(pet (index %d)(time %d)(heal %d)(autopick %d))", &PetIndex, &GetPetTime, &Heal, &AutoPick) == 4)
			{
	...
	if (filen != NULL)
	{
		...
		{
			int ArmorGetPrefix = 0, ArmorGetDef = 0, BofPrefix = 0, ImpPrefix = 0, GetA = 0, GetM = 0, GetTOA = 0, GetUpg = 0, GetPrefix = 0, MixPrefix = 0, MixInfo = 0; char mixnotice[BUFSIZ];
	...
	if (filem != NULL)
	{
		...
		while (fgets(line, sizeof line, filem) != NULL)
		{
			...
			if (sscanf(line, "(notice (day '%[a-z | A-Z | 0-9/<>|.,~*;`:!^+%&=?_-£#$€]')(time '%[a-z | A-Z | 0-9/<>|.,~*;`:!^+%&=?_-£#$€]')(msg '%[a-z | A-Z | 0-9/<>|.,~*;`:!^+%&=?_-£#$€]'))", &day, &time, &notice) == 3)
			{
	...
	if (files != NULL)
	{
		...
		{
			char day[BUFSIZ],time[BUFSIZ],notice[BUFSIZ]; int MobIndex = 0, MobAmount = 0, MobMap = 0, MobX = 0, MobY = 0, MobDis = 0;
			if (sscanf(line, "(summon (index %d)(amount %d)(map %d)(x %d)(y %d)(disappear %d)(day '%[a-z | A-Z | 0-9/<>|.,~*;`:!^+%&=?_-£#$€]')(time '%[a-z | A-Z | 0-9/<>|.,~*;`:!^+%&=?_-£#$€]')(msg '%[a-z | A-Z | 0-9/<>|.,~*;`:!^+%&=?_-£#$€]'))", &MobIndex, &MobAmount, &MobMap, &MobX, &MobY, &MobDis, &day, &time, &notice) == 9)
			{
	...
	if (DailyQuestx != NULL)
	{
		...
		{
			int DailyQuesty = 0, DailyLevel = 0, DailyMonster = 0, DailyAmount = 0, DailyRepeat = 0, DailyTime = 0, DailyItem = 0, DailyIAmount = 0, DailyExp = 0;

			if (sscanf(line, "(dailyquest (quest %d)(level %d)(monster %d)(amount %d)(maxrepeat %d)(timelimit %d)(item %d)(amount %d)(exp %d))", &DailyQuesty, &DailyLevel, &DailyMonster, &DailyAmount, &DailyRepeat, &DailyTime, &DailyItem, &DailyIAmount, &DailyExp) == 9)
			{
	...
	if (oItemShop != NULL)
	{
		...
		{
			__int16 BillCode = 0; char Type = 0, Priority = 0, Discount = 0; int Price = 0, ItemIndex = 0, Amount = 0; 
			if (sscanf(line, "(itemshop (billcode %d)(priority %d)(type %d)(price %d)(discount %d)(itemindex %d)(amount %d))", &BillCode, &Priority, &Type, &Price, &Discount, &ItemIndex, &Amount) == 7)
			{
				...
				Tools->Compile(ItemShopPacket+(count*9),"wbbdb",BillCode,Priority,Type,Price,Discount);
