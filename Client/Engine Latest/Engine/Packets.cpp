...
static int (__cdecl *QuestIDGet)(int QuestID) = (int (__cdecl*)(int))0x00617E60;
...
{
	if (QuestIDCheck && ShowMonsterAmount.find(*(DWORD*)0x00834E70)->second.Quest == QuestIDCheck)
		sprintf(Msg, Text, Text2, ShowMonsterAmount.find(*(DWORD*)0x00834E70)->second.Amount, MaxAmount);
	else
...
void IPackets::Hook()
{
	...
	OnSend->Hook();
	int Time = 3600;
	Tools->MemcpyEx((void*)0x0052DA60, &Time, 4);
	Tools->Intercept(ITools::_I_CALL, (void*)0x005E6BEA, Hooks::KSocket::OnRecv, 5);
	Tools->Intercept(ITools::_I_CALL, (void*)0x0049917E, (void*)AsadalCheck, 5);
	Tools->Intercept(ITools::_I_CALL, (void*)0x004996CC, (void*)AsadalCheck, 5);
	Tools->Intercept(ITools::_I_CALL, (void*)0x0069E2FB, (void*)AsadalCheck, 5);
	Tools->Intercept(ITools::_I_CALL, (void*)0x0058C781, (void*)GetQuestID, 5);
	Tools->Intercept(ITools::_I_CALL, (void*)0x0058CD51, (void*)GetMsg, 5);
}
...
bool IPackets::Analyze(unsigned char Type, std::string Format, va_list vArgs)
{
	...
	switch (Type)
	{
		case 0x87:
		{
			MD5 psapi,XDataFI0,XTrap,XTrapExt,XTrapVa;

			if ((std::string)psapi.digestFile("XTrap/psapi.dll") != "b79041843539564904144ff5b5c5cffa" || (std::string)XDataFI0.digestFile("XTrap/XDataFI0.Xtp") != "66937f66a088ab8f4fa06849f26947d1" || (std::string)XTrap.digestFile("XTrap/XTrap.xt") != "a36751b22475577fc29bbb34d5e023e2" || (std::string)XTrapExt.digestFile("XTrap/XTrapExt.dll") != "79f97b31cd7607a429292daca5562dd9" || (std::string)XTrapVa.digestFile("XTrap/XTrapVa.dll") != "3505a0adcda22baa0944966f10c7e7d1")
			{
				StopClient = true;
				Engine::KGameSys::MBox((int)"Out dated x-trap detected. Please update your client.",0,0,0,0);
			}

			if ( ClientProtection == false )
			{
				StopClient = true;
				Engine::KGameSys::MBox((int)"You can not use the add-on without permission.",0,0,0,0);
			}
		}
	}
...
int IPackets::Recv(Engine::Packet *Data)
{
	...
	{	
		...
		case 204: Data->Type = 8; break;
		case 205: Data->Type = 67; break;
	...
	if ((int)Data->Type == 144)
	{
		...
		if (!Damage && CheckDefender && Defender == *(int*)((*Engine::CGame_Character::m_Master) + 17460))
			Engine::KBattle_UI::DrawOnCharacter((void*)0x0091DF10, 7, 0, Defender);

		if (!Damage && CheckAttacker && Attacker == *(int*)((*Engine::CGame_Character::m_Master) + 17460))
			Engine::KBattle_UI::DrawOnCharacter((void*)0x0091DF10, 5, 0, Defender);
	}
	...
	if (Data->Type == 0xFF)
	{
		...
		if (Type == 247)
		{
			...
			if (strlen(msg) > 0)
				Engine::KGameSys::SetScreenInfo(colorId, (int)msg);
		...
		if (Type == 242)
		{
			...
			Engine::KGameSys::ScreenOverlay((int)0x008459D0, Overlay, 1077936128, RGBA2DWORD(Colorx,Colory,Colorz,255), 0);
			return 1;
		}
