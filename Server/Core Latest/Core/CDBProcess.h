int __cdecl CDBProcess(char *Data)
{
	if (Data[2] == 25)
	{
		...
		CPacket::xRead((void*)(Data + 3), "dbdbbdsd", &PID, &Type, &MID, &xType, &Status, &RPID, &Name, &IID);
	...
	if (Data[2] == 50)
	{
		...
		CPacket::xRead((void*)(Data + 3), "ddddddddddd", &PID, &HP, &HK, &HD, &DKPT, &DKPW, &PLT, &PLW, &SVT, &SVW, &RP);	
	...
	if (Data[2] == 52)
	{
		...
		if (PID && Type && Remain)
			SetBuffx[PID+4000000000+(Type*1000000)] = Remain;
				...
				if (Undefined::CheckValues(&CheckValue, Checkx))
				{
					...
					{	
						...
						*(DWORD*)(MyItem+72) = 0;
						...
						CPlayer::Write(IPlayer.GetOffset(),0xFF,"ddd",230,IID,(PetLifeCheck.find(IID)->second.Time-(int)time(0))*1000);
					}
				}
			}
		}
	...
	if (Data[2] == 55)
	{
		int IID = 0, ItemStat = 0; Interface<ITools> Tools;
	...
	if (Data[2] == 56)
	{
		int PID = 0, QuestID = 0, Clear = 0, QuestTime = 0, QuestRepeat = 0, MonsterCount = 0;
		CPacket::xRead((void*)(Data + 3), "dddddd", &PID, &QuestID, &Clear, &QuestTime, &QuestRepeat, &MonsterCount);
		...
		if (PID && QuestID && QuestTime)
		{
			...
			int Player = CPlayer::ScanPlayer(PID);
			IChar IPlayer((void*)Player);
			if (Player && IPlayer.IsOnline()) CPlayer::Write((void*)Player,255,"dddd",222,PID,QuestID,MonsterCount);
		}
