...
int __cdecl MyUpdateProperty(int Player, int Type, int InOut, signed __int64 Exp)
{
	...
	if (Type == 25 && IPlayer.IsOnline() && InOut)
	{
		...
		if (LevelGap.count(IPlayer.GetLevel()) && LevelGap.find(IPlayer.GetLevel())->second > 0 && Exp > 100)
...
{
	IChar IPlayer((void*)Player);
	CPlayer::Write(IPlayer.GetOffset(), 69, "bwwww", 1, IPlayer.GetHth(), IPlayer.GetCurHp(), CChar::GetMaxHP(Player), CChar::GetResist(IPlayer.GetOffset(), 4));
	CPlayer::Write(IPlayer.GetOffset(), 69, "bwwww", 44, (CChar::GetHth(Player) - IPlayer.GetHth()), IPlayer.GetCurHp(), CChar::GetMaxHP(Player), CChar::GetResist(IPlayer.GetOffset(), 4));
}
...
int __fastcall IsNormal(int Player, void *edx)
{
	...
	if (IPlayer.IsValid())
	{
		...
		if (IPlayer.GetClass() < 4 && CChar::IsGState((int)IPlayer.GetOffset(),512))
		{
...
int __fastcall IsStateCheck(int Item, void *edx, int Value)
{
	...
	if (IItem.CheckIndex() >= 3381 && IItem.CheckIndex() <= 3383 && !CheckHaninMirror.find(*(DWORD*)(Item + 32))->second)
		CheckHaninMirror[*(DWORD*)(Item + 32)] = IItem.GetIID();

	if (IItem.CheckIndex() >= 2986 && IItem.CheckIndex() <= 3009 && IItem.GetInfo() & 1)
	{
		int GetGrade = (GetItemStat.find(IItem.GetIID())->second % 10000) / 100;
		
		if (TrigramGrade.count(*(DWORD*)(Item + 32)) && GetGrade < TrigramGrade.find(*(DWORD*)(Item + 32))->second)
	...
	if (IItem.CheckIndex() >= 2986 && IItem.CheckIndex() <= 3009 && IItem.GetInfo() & 1)
	{
		...
		if (IItem.CheckIndex() == 2986) TrigramHP[*(DWORD*)(Item + 32)] = TriagramStats[0][GetCurrentGrade];
		if (IItem.CheckIndex() == 2994) TrigramHP[*(DWORD*)(Item + 32)] = TriagramStats[1][GetCurrentGrade];
		if (IItem.CheckIndex() == 3002) TrigramHP[*(DWORD*)(Item + 32)] = TriagramStats[2][GetCurrentGrade];
		if (IItem.CheckIndex() == 2987) TrigramMP[*(DWORD*)(Item + 32)] = TriagramStats[3][GetCurrentGrade];
		if (IItem.CheckIndex() == 2995) TrigramMP[*(DWORD*)(Item + 32)] = TriagramStats[4][GetCurrentGrade];
		if (IItem.CheckIndex() == 3003) TrigramMP[*(DWORD*)(Item + 32)] = TriagramStats[5][GetCurrentGrade];
		if (IItem.CheckIndex() == 2988) TrigramAtk[*(DWORD*)(Item + 32)] = TriagramStats[6][GetCurrentGrade];
		if (IItem.CheckIndex() == 2996) TrigramAtk[*(DWORD*)(Item + 32)] = TriagramStats[7][GetCurrentGrade];
		if (IItem.CheckIndex() == 3004) TrigramAtk[*(DWORD*)(Item + 32)] = TriagramStats[8][GetCurrentGrade];
		if (IItem.CheckIndex() == 2989) TrigramStr[*(DWORD*)(Item + 32)] = TriagramStats[9][GetCurrentGrade];
		if (IItem.CheckIndex() == 2997) TrigramStr[*(DWORD*)(Item + 32)] = TriagramStats[10][GetCurrentGrade];
		if (IItem.CheckIndex() == 3005) TrigramStr[*(DWORD*)(Item + 32)] = TriagramStats[11][GetCurrentGrade];
		if (IItem.CheckIndex() == 2990) TrigramAgi[*(DWORD*)(Item + 32)] = TriagramStats[12][GetCurrentGrade];
		if (IItem.CheckIndex() == 2998) TrigramAgi[*(DWORD*)(Item + 32)] = TriagramStats[13][GetCurrentGrade];
		if (IItem.CheckIndex() == 3006) TrigramAgi[*(DWORD*)(Item + 32)] = TriagramStats[14][GetCurrentGrade];
		if (IItem.CheckIndex() == 2991) TrigramInt[*(DWORD*)(Item + 32)] = TriagramStats[15][GetCurrentGrade];
		if (IItem.CheckIndex() == 2999) TrigramInt[*(DWORD*)(Item + 32)] = TriagramStats[16][GetCurrentGrade];
		if (IItem.CheckIndex() == 3007) TrigramInt[*(DWORD*)(Item + 32)] = TriagramStats[17][GetCurrentGrade];
		if (IItem.CheckIndex() == 2992) TrigramWis[*(DWORD*)(Item + 32)] = TriagramStats[18][GetCurrentGrade];
		if (IItem.CheckIndex() == 3000) TrigramWis[*(DWORD*)(Item + 32)] = TriagramStats[19][GetCurrentGrade];
		if (IItem.CheckIndex() == 3008) TrigramWis[*(DWORD*)(Item + 32)] = TriagramStats[20][GetCurrentGrade];
		if (IItem.CheckIndex() == 2993) TrigramHth[*(DWORD*)(Item + 32)] = TriagramStats[21][GetCurrentGrade];
		if (IItem.CheckIndex() == 3001) TrigramHth[*(DWORD*)(Item + 32)] = TriagramStats[22][GetCurrentGrade];
		if (IItem.CheckIndex() == 3009) TrigramHth[*(DWORD*)(Item + 32)] = TriagramStats[23][GetCurrentGrade];
	}

	if (IItem.CheckIndex() >= 3018 && IItem.CheckIndex() <= 3020 && IItem.GetInfo() & 1)
	{
		int GetTaegeukPrefix = GetItemStat.find(IItem.GetIID())->second / 1000;
		if (GetTaegeukPrefix) Taegeuk[*(DWORD*)(Item + 32)] = GetTaegeukPrefix;
	}

	if ((IItem.CheckIndex() >= 1747 && IItem.CheckIndex() <= 1762) || (IItem.CheckIndex() >= 1870 && IItem.CheckIndex() <= 1877) || (IItem.CheckIndex() >= 2004 && IItem.CheckIndex() <= 2007) || (IItem.CheckIndex() >= 2421 && IItem.CheckIndex() <= 2423) || (IItem.CheckIndex() >= 2550 && IItem.CheckIndex() <= 2653))
...
void ExpMultiplier()
{
	...
	int InventoryLimit = 72;
	int ItemTimer = 1000;
	Memory->Copy((void*)0x0042670F, &ItemTimer, 4);
	Memory->Copy((void*)0x0042F212, &EggGradeLimit, 1);
	Memory->Copy((void*)0x00455FD0, &InventoryLimit, 1);
	Memory->Copy((void*)0x004560E1, &InventoryLimit, 1);
	Memory->Copy((void*)0x0045DE84, &InventoryLimit, 1);
	Memory->Copy((void*)0x0045E4D6, &InventoryLimit, 1);
	Memory->Copy((void*)0x0045EF08, &InventoryLimit, 1);
	Memory->Copy((void*)0x0045F6D5, &InventoryLimit, 1);
	Memory->Copy((void*)0x00464FDA, &InventoryLimit, 1);
	Memory->Copy((void*)0x00469759, &InventoryLimit, 1);
