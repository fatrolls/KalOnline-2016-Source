void __fastcall MAILProcess(void *Player, void *edx, int Type, int pPacket, int pPos)
{
	...
	if (IPlayer.IsOnline() && Type == 7 && !IPlayer.IsBuff(343))
	{
		...
		char Packet[63] = {0};
		int Check, nMID=0, nIID=0; char byMType=0; unsigned __int16 wLen = 0; char Msg[512] = {0};
		Check = CPacket::xRead((void *)pPacket, "dbdw", &nMID, &byMType, &nIID, &wLen);
		Check = CPacket::xRead((void *)Check, "m", &Msg, wLen);
		int Index = 0, Prefix = 0, MaxEnd = 0, CurEnd = 0, Info = 0, Num = 0, XHit = 0, XDefense = 0, XAttack = 0, XMagic = 0, XDodge = 0, Protect = 0, UpgrLevel = 0, SetGem = 0;
		...
		query << "SELECT [Index],[MaxEnd],[CurEnd],[Info],[Num],[XHit],[XDefense],[XAttack],[XMagic],[XDodge],[Protect],[UpgrLevel],[SetGem],[Prefix] FROM [Item] WHERE [IID] = '" << nIID << "'";
		...
		int ItemStat = GetItemStat.find(nIID)->second;
		...
		if (Index)
		{
			Tools->Compile(Packet,"wdbddbbbbbbbbwbbbbbdbwwwwbbbbbbbbbbdbbwbb",Index,nIID,Prefix,Info,Num,MaxEnd,CurEnd,SetGem,XAttack,XMagic,XDefense,XHit,XDodge,Protect,UpgrLevel,UpgrLevel, x, y, z, remaintime, dsstype, phyatk, magatk, def, absorb, eva, otp, hpinc, mpinc, str, hp, intel, wis, agi, PerfShotCheck, QigongGrade, dg1stat, dg1type, a, dg2stat, dg2type);
			CPlayer::Write(Player, 19, "bdbdwmm", 2, nMID, byMType, nIID, wLen, &Msg, wLen, Packet, 63);
			return;
		}
	}
