void __fastcall ExecuteSkill(void *pSkill, void* edx, signed int SkillID, int pPacket, int pPos)
{
	...
	if (IPlayer.IsValid() && !CChar::IsGState((int)IPlayer.GetOffset(),512))
	{
		...
		{
			CooldownCheck = CooldownTable.find(IPlayer.GetPID()+4000000000+(SkillID*1000000))->second.Cooldown;
			DelayCheck = CooldownTable.find(IPlayer.GetPID()+4000000000+(SkillID*1000000))->second.Delay;
		}
		...
		if (DelayCheck > GetTickCount())
		{
			...
			CooldownTable[IPlayer.GetPID()+4000000000+(SkillID*1000000)].Delay = GetTickCount()+DelayTime;
		}
		...
		if (CooldownCheck > GetTickCount())
		{
			...
			CooldownTable[IPlayer.GetPID()+4000000000+(SkillID*1000000)].Cooldown = GetTickCount()+CdTime;
		}
