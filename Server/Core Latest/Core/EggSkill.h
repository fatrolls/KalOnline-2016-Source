void __fastcall ExecuteTransformSkill(void *pSkill, void* edx, signed int SkillID, int pPacket, int pPos)
{
	...
	DWORD CdTime = 0, CooldownCheck = 0, DelayTime = 0, DelayCheck = 0;
	...
	if (IPlayer.IsValid() && CChar::IsGState((int)IPlayer.GetOffset(),512))
	{
		...
		{
			CooldownCheck = EggCooldownTable.find(IPlayer.GetPID()+4000000000+(SkillID*1000000))->second.EggCooldown;
			DelayCheck = EggCooldownTable.find(IPlayer.GetPID()+4000000000+(SkillID*1000000))->second.EggDelay;
		}
		...
		if (DelayCheck > GetTickCount())
		{
			IPlayer.SystemMessage("Invalid egg skill prepare time detected!", TEXTCOLOR_RED);
			return;
		} else {
			EggCooldownTable[IPlayer.GetPID()+4000000000+(SkillID*1000000)].EggDelay = GetTickCount()+DelayTime;
		}

		if (CooldownCheck > GetTickCount())
		{
			IPlayer.SystemMessage("Invalid egg skill cooldown time detected!", TEXTCOLOR_RED);
			return;
		} else {
			EggCooldownTable[IPlayer.GetPID()+4000000000+(SkillID*1000000)].EggCooldown = GetTickCount()+CdTime;
		}
